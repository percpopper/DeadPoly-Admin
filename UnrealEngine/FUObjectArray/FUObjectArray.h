...
public:
	...
	FORCEINLINE FUObjectItem const* GetObjectPtr(int32 Index) const
	{
		...
		if (!IsValidIndex(Index)) {
			LOGFAIL("IsValidIndex Returned False For Index -> (%d)\n\n", Index);
			return NULL;
		}

		if (ChunkIndex >= NumChunks) {
			LOGFAIL("ChunkIndex (%d) >= NumChunks (%d)\n\n", ChunkIndex, NumChunks);
			return NULL;
		}

		if (Index > MaxElements) {
			LOGFAIL("Index (%d) > MaxElements (%d)\n\n", Index, MaxElements);
			return NULL;
		}
	...
	T FindUObject(const char* NameToFind)
	{
		for (int CurrentElement = 0; CurrentElement < NumElements; CurrentElement++)
		{
			...
			if (ObjectItem && ObjectItem->Object->GetFullName() == NameToFind) {
				LOGSUCCESS("Object with the name \"%s\" was the %i object found.\n\n", NameToFind, CurrentElement);

				return static_cast<T>(ObjectItem->Object);
			}
		}

		LOGFAIL("All %i objects were checked for the name %s with no result.\n\n", NumElements, NameToFind);
